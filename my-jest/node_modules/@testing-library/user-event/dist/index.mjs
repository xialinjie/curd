...
var DataTransferStub = class {
  ...
  getData(format) {
    var _a3;
    const match = (_a3 = this.items.find(getTypeMatcher(format, true))) != null ? _a3 : this.items.find(getTypeMatcher(format, false));
    let text = "";
...
var Window = Symbol("Window reference");
var _a;
var ClipboardItemStub = class {
  ...
  getType(type3) {
    return __async(this, null, function* () {
      ...
      return data instanceof this[Window].Blob ? data : new this[Window].Blob([data], { type: type3 });
    });
  }
};
_a = Window;
...
var _a2;
var ClipboardStub = class extends window.EventTarget {
  constructor() {
    super(...arguments);
    this.items = [];
    this[_a2] = window;
  ...
  readText() {
    return __async(this, null, function* () {
      ...
      for (const item of this.items) {
        ...
        if (type3) {
          text += yield item.getType(type3).then((b) => readBlobText(b, this[Window].FileReader));
        }
      }
      return text;
    });
...
_a2 = Window, ClipboardStubControl;
function createClipboardItem(window2, ...blobs) {
  const data = Object.fromEntries(blobs.map((b) => [
  ...
  if (typeof window2.ClipboardItem === "undefined") {
    ...
    return new window2.ClipboardItem(data);
  }
}
  ...
  return __async(this, null, function* () {
    ...
    for (const item of items) {
      for (const type3 of item.types) {
        dt.setData(type3, yield item.getType(type3).then((b) => readBlobText(b, window2.FileReader)));
      }
    }
    return dt;
  });
...
function getSanitizedMaxLength(element) {
  ...
  const attr = (_a3 = element.getAttribute("maxlength")) != null ? _a3 : "";
...
function isValidNumberInput(value) {
  ...
  return !(/[^\d.\-e]/.test(value) || Number((_a3 = value.match(/-/g)) == null ? void 0 : _a3.length) > 2 || Number((_b = value.match(/\./g)) == null ? void 0 : _b.length) > 1 || valueParts[1] && !/^-?\d*$/.test(valueParts[1]));
}
...
function setFiles(el, files) {
  var _a3;
  (_a3 = el[fakeFiles]) == null ? void 0 : _a3.restore();
    ...
    value: __spreadProps(__spreadValues(__spreadValues({}, prototypeDescriptors.value), objectDescriptors.value), {
      ...
      set(v) {
        ...
        if (v === "") {
          ...
          (_a4 = objectDescriptors.value.set) == null ? void 0 : _a4.call(el, v);
        }
      }
    }),
...
function isDisabled(element) {
  ...
  for (let el = element; el; el = el.parentElement) {
    if (isElementType(el, [
      ...
      if (el.hasAttribute("disabled") && !((_a3 = el.querySelector(":scope > legend")) == null ? void 0 : _a3.contains(element))) {
        return true;
      }
...
function walkNodes(node, direction, callback) {
  ...
  for (; ; ) {
    ...
    } else if (node.parentNode && (!isElement(node.parentNode) || !isContentEditable(node.parentNode) && node.parentNode !== ((_a3 = node.ownerDocument) == null ? void 0 : _a3.body))) {
...
function setSelectionRange(element, anchorOffset, focusOffset) {
  ...
  if (isContentEditable(element) && ((_a3 = element.firstChild) == null ? void 0 : _a3.nodeType) === 3) {
    return setSelection({
  ...
  if (hasOwnSelection(element)) {
    ...
    if (contenteditable) {
      if (!selection.isCollapsed) {
        const focusNode = ((_a3 = contenteditable.firstChild) == null ? void 0 : _a3.nodeType) === 3 ? contenteditable.firstChild : contenteditable;
...
function getInputRange(focusNode) {
  ...
  if (typeAndSelection.type === "input") {
    ...
    return (_a3 = typeAndSelection.selection) == null ? void 0 : _a3.getRangeAt(0);
  }
}
function setSelection({
  ...
  (_b = (_a3 = anchorNode.ownerDocument) == null ? void 0 : _a3.getSelection()) == null ? void 0 : _b.setBaseAndExtent(anchorNode, anchorOffset, focusNode, focusOffset);
}
...
function selectAll(target) {
  ...
  const focusNode = (_a3 = getContentEditable(target)) != null ? _a3 : target.ownerDocument.body;
  setSelection({
    focusNode,
    anchorOffset: 0,
...
function readTag(text, pos, startBracket, context) {
  ...
  const descriptor = escapedDescriptor ? text[pos] : (_a3 = text.slice(pos).match(startBracket === "{" ? /^\w+|^[^}>/]/ : /^\w+/)) == null ? void 0 : _a3[0];
  assertDescriptor(descriptor, text, pos, context);
  pos += descriptor.length;
...
function setLevelRef(config, level) {
  var _a3;
  (_a3 = config[Level]) != null ? _a3 : config[Level] = {};
  config[Level][level] = {};
}
function getLevelRef(config, level) {
  var _a3;
  return (_a3 = config[Level]) == null ? void 0 : _a3[level];
}
...
function assignPositionInit(obj, {
  ...
  var _a3, _b, _c, _d;
  assignProps(obj, {
    x: (_a3 = x != null ? x : clientX) != null ? _a3 : 0,
...
behavior.keydown = (event, target, config) => {
  var _a3, _b;
  return (_b = (_a3 = keydownBehavior[event.key]) == null ? void 0 : _a3.call(keydownBehavior, event, target, config)) != null ? _b : combinationBehavior(event, target, config);
};
var keydownBehavior = {
  ...
  End: (event, target) => {
    if (isElementType(target, ["input", "textarea"]) || isContentEditable(target)) {
      return () => {
        var _a3, _b;
        const newPos = (_b = (_a3 = getValue(target)) == null ? void 0 : _a3.length) != null ? _b : 0;
...
behavior.keyup = (event, target, config) => {
  var _a3;
  return (_a3 = keyupBehavior[event.key]) == null ? void 0 : _a3.call(keyupBehavior, event, target, config);
};
var keyupBehavior = {
  " ": (event, target, config) => {
...
behavior.paste = (event, target, config) => {
  if (isEditable(target)) {
    return () => {
      var _a3;
      const insertData = (_a3 = event.clipboardData) == null ? void 0 : _a3.getData("text");
      if (insertData) {
...
function dispatchEvent(config, target, event, preventDefault = false) {
  ...
  } : (_b = (_a3 = behavior)[type3]) == null ? void 0 : _b.call(_a3, event, target, config);
...
function getUISelection(element) {
  ...
  const sel = (_c = element[UISelection]) != null ? _c : {
    anchorOffset: (_a3 = element.selectionStart) != null ? _a3 : 0,
...
function trackOrSetValue(element, v) {
  var _a3, _b, _c;
  (_b = (_a3 = element[TrackChanges]) == null ? void 0 : _a3.tracked) == null ? void 0 : _b.push(v);
...
function endTrackValue(element) {
  ...
  const isJustReactStateUpdate = ((_a3 = changes == null ? void 0 : changes.tracked) == null ? void 0 : _a3.length) === 2 && changes.tracked[0] === changes.previousValue && changes.tracked[1] === changes.nextValue;
...
function preKeydownBehavior(config, { key }, element) {
  ...
  if (isModifierKey(key)) {
    ...
    if (key === "AltGraph") {
      const ctrlKeyDef = (_a3 = config.keyboardMap.find((k) => k.key === "Control")) != null ? _a3 : { key: "Control", code: "Control" };
...
function keyboardKeyAction(_0, _1) {
  return __async(this, arguments, function* (config, { keyDef, releasePrevious, releaseSelf, repeat }) {
    const { document: document2, keyboardState } = config;
...
function getActive(document2) {
  var _a3;
  return (_a3 = getActiveElement(document2)) != null ? _a3 : document2.body;
}
...
function hasKeyPress(keyDef, config) {
  var _a3;
  return (((_a3 = keyDef.key) == null ? void 0 : _a3.length) === 1 || keyDef.key === "Enter") && !config.keyboardState.modifiers.Control && !config.keyboardState.modifiers.Alt;
}
...
function parseKeyDef(keyboardMap, text) {
  ...
  do {
    ...
    const keyDef = (_a3 = keyboardMap.find((def) => {
      var _a4, _b;
      if (type3 === "[") {
        return ((_a4 = def.code) == null ? void 0 : _a4.toLowerCase()) === descriptor.toLowerCase();
      } else if (type3 === "{") {
...
function firePointerEvent(config, target, type3, {
  ...
  init.buttons = getMouseButtons(...config.pointerState.pressed.filter((p) => p.keyDef.pointerType === pointerType).map((p) => {
    var _a3;
    return (_a3 = p.keyDef.button) != null ? _a3 : 0;
  }));
...
function down(config, pointerName, { keyDef, node, offset, target, coords }, targetIsDisabled) {
  ...
  if (((_a3 = pointerState.activeClickCount) == null ? void 0 : _a3[0]) !== name) {
...
function pointerAction(config, actions) {
  return __async(this, null, function* () {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      const target = (_a3 = action.target) != null ? _a3 : getPrevTarget(pointerName, config.pointerState);
...
function copy() {
  return __async(this, null, function* () {
    ...
    const target = (_a3 = doc.activeElement) != null ? _a3 : doc.body;
...
function upload(element, fileOrFiles) {
  return __async(this, null, function* () {
    ...
    const fileDialog = () => {
      var _a3;
      if (files.length === ((_a3 = input2.files) == null ? void 0 : _a3.length) && files.every((f, i) => {
        var _a4;
        return f === ((_a4 = input2.files) == null ? void 0 : _a4.item(i));
      })) {
        return;
      }
...
function setupMain(options = {}) {
  ...
  const view = (_a3 = config.document.defaultView) != null ? _a3 : window;
...
function getDocument(options, node) {
  var _a3, _b;
  return (_b = (_a3 = options.document) != null ? _a3 : node && getDocumentFromNode(node)) != null ? _b : document;
}
